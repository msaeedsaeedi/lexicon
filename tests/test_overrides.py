...
def _write_overrides(directory: Path, blocked: list[dict[str, str]], fixes: list[dict[str, str]]) -> None:
    directory.mkdir()
...
def test_overrides_block_lexemes_and_fix_definitions(tmp_path: Path) -> None:
    ...
    result = CliRunner().invoke(
        app,
        ["build", "--input", "data/seed", "--overrides", str(overrides), "--output", str(tmp_path / "artifacts")],
    )

    assert result.exit_code == 0, result.output
    records = [json.loads(line) for line in (tmp_path / "artifacts" / "lexicon-en-core-0.1.0.jsonl").read_text().splitlines()]
